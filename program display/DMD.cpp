...
void DMD::shiftOutFast( byte _val){
for(byte b=0;b<8;b++){
  if((_val&0x80)==0x80) PORTD |= _BV(PD4); 
  else  PORTD &= ~ _BV(PD4); 
  _val=(_val<<1)|(_val>>7);
  PORTD |= _BV(PD2);  //1
  PORTD &= ~_BV(PD2);  //0
...
void DMD::scanDisplayBySPI()
{
    ...
    if(true)//digitalRead( PIN_OTHER_SPI_nCS ) == HIGH )
    {
        //SPI transfer pixels to the display hardware shift registers
        int rowsize=DisplaysTotal<<2;
        int offset=rowsize * bDMDByte;
        for (int i=0;i<rowsize;i++) {
            shiftOutFast(bDMDScreenRAM[offset+i+row3]);
            shiftOutFast(bDMDScreenRAM[offset+i+row2]);
            shiftOutFast(bDMDScreenRAM[offset+i+row1]);
            shiftOutFast(bDMDScreenRAM[offset+i]);
        }
